...
function compareKeyInfo(keyInfo: KeyInfo, sklKeyInfo: KeyInfo) {
  ...
  if (
    keyInfo.SHA256Fingerprints.length !== sklKeyInfo.SHA256Fingerprints.length
  ) {
...
async function verifyKeyLists(
  ...
  const keyListInfo = await Promise.all(
    keyList.map(async (key, i) => {
      return {
        ...
        SHA256Fingerprints: (
          await getSHA256Fingerprints(key.PublicKey)
        ).map((sha256fingerprint: string) => sha256fingerprint.toLowerCase()),
        Primary: i === 0 ? 1 : 0,
        Flags: key.Flags,
      };
    })
  ...
  const signedKeyListInfo = signedKeyListData.map((keyInfo) => {
    return {
      ...
      SHA256Fingerprints: keyInfo.SHA256Fingerprints.map(
        (sha256fingerprint: string) => sha256fingerprint.toLowerCase()
      ),
    };
  });
...
async function verifyEpoch(
  ...
  const { Name, Proof, Revision, Neighbors } = await api(
  ...
  switch (certificate.notBefore.toJSON().type) {
    ...
    default:
      throw new Error(
        `Certificate's notBefore date is invalid (type = ${
          certificate.notBefore.toJSON().type
        })`
      );
  }
...
> {
  ...
  for (let i = 0; i < addresses.length; i++) {
    ...
    if (ktBlob !== undefined && ktBlob !== null) {
      ...
      const fetchedSKLs = await getParsedSignedKeyLists(
        api,
        localEpoch.EpochID,
        email,
        false
    ...
    let errorResponse;
    try {
      verifiedEpoch = await api(
        getLatestVerifiedEpoch({ AddressID: address.ID })
      );
    } catch (err) {
    ...
    const newerSKLs = await getParsedSignedKeyLists(
      api,
      verifiedEpochData.EpochID,
      email,
      true
    );
    ...
    if (newerEpochs.length === 1) {
      const newestEpoch = newerSKLs.find(
    ...
    if (
      ...
      address.SignedKeyList.MinEpochID > newerEpochs[0].EpochID
    ) {
